...
FilePlayer.prototype.renderElement = function(_parent, _i) {
  var buttonPlay = $('<button data-role="button" id="playCondition'+_i+'" name="'+_i+'  width="auto"">' + this.localizer.getFragment(this.language, 'playButton')  + '</button>');
  buttonPlay.bind( "click", (function(event, ui) {
    this.genericAudioControl.play(parseInt(event.target.name));
  }).bind(this));
    
  var buttonPause = $('<button data-role="button" id="pause" width="auto">' + this.localizer.getFragment(this.language, 'pauseButton')  + '</button>');
